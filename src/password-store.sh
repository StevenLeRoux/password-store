...
source "/usr/local/Cellar/pass/1.7.3/lib/password-store/platform.sh"
...
cmd_git() {
	...
	while getopts p: FLAG; do
		case $FLAG in
			p) PREFIX=$PREFIX/$OPTARG ; INNER_GIT_DIR=$PREFIX ;;
			\?) ;;
	esac done
	shift $(expr $OPTIND - 1 )
	set_git "$PREFIX/"
...
SYSTEM_EXTENSION_DIR="/usr/local/lib/password-store/extensions"
cmd_extension() {
